...
class DebateManager:
    ...
    def __init__(
        ...
        # Debate history – each entry is a dict describing one speech
        self.history: list[dict] = []
        # Accumulating user interrupts; cleared after each round
    ...
    def run_facilitation(self) -> None:
        """Facilitator gathers context and opens the debate."""
...
Keep the total under 450 words. Be engaging and neutral."""
    ...
    def run_round(self) -> None:
        """Each selected debater speaks once in order."""
    ...
    def run_summary(self) -> None:
        """Facilitator produces agreed conclusions and open questions."""
        self._begin("summary")
...
## 📋 Overall Assessment
What did this debate accomplish? What was the quality of discourse? What should be explored next?
...
Be specific—reference actual statements from the transcript. Keep each section concise."""
    ...
    def _stream_speaker(
        ...
        # Persist to history
        entry: dict = {
            "role": "facilitator" if speaker in ("facilitator", "summary") else "debater",
            "speaker": speaker,
            "text": full_text,
            "ts": time.time(),
        }
    ...
    def _build_debater_prompt(self, debater: str, round_user_inputs: list[str]) -> str:
...
Write your Round {self.current_round} argument now:"""
    ...
    def _full_transcript(self) -> str:
        """Complete, untruncated transcript for the summary prompt."""
        parts = []
        for entry in self.history:
            role = entry["role"]
            speaker = entry["speaker"].upper()
